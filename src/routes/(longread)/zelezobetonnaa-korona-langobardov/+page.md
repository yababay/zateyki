...
<script>
    ...
    import src   from './lang-01.webp'
    export let data
    const { title, description, banner, image } = data
    const caption = 'Территории лангобардов.'
</script>
...
герцогов, но ничего особо интересного и яркого источники о них не
...
отрезало: Тёмные века-с. Тем не менее, как нас убеждают, покуражились
...
возвращений не наблюдается. Наблюдается расширение НАТО на восток,
...
под предлогом того, что их «участки» «должным образом не зарегистрированы». Ну
...
[Публикация в Telegraph](https://telegra.ph/ZHelezobetonnaya-korona-langobardov-12-08)
